...
| 11 | LinkedIn icon in footer | Opens developers LinkedIn page in a new tab | N/A |
...
|  Test  |  Purpose  |  Expected Result  |  Example  |
|:---:|-----------------| ---------------- |:-----:|
| 01 | Edit Recipe Form | Edit recipe form should have the same fields available as the add a recipe form | see test 02 |
...
|  Test  |  Purpose  |  Expected Result  |  Example  |
|:---:|-----------------| ---------------- |:-----:|
| 01 | Profile Information | First Name, profile image, Recipes created & recipes saved display at the top of the page | [View](documentation/testing/functionality/profile/information.png) |
| 02 | Edit Profile Button | Triggers Edit Profile Modal | see test 03 |
| 03 | Edit Profile Modal | User can update there image URL by entering the URL into the field provided and clicking update. Clicking close will close the edit profile modal. If the URL entered does not direct to an image, then a default image will be displayed. | [View](documentation/testing/functionality/profile/profile-image.gif) |
...
| 06 | Delete Profile Modal | User is asked to confirm whether they wish to delete their profile and a warning to let the user know any recipes they have uploaded will not be deleted is displayed. Clicking delete will remove the user from the database | [View](documentation/testing/functionality/profile/delete-profile.gif) |
...
| 10 | Failed deployment to Heroku | The app was failing to build by Heroku. After reviewing the build log on Heroku I discovered my compiled slug size was too large. | I originally began by compressing the file size of the GIF's, however, after further research, I have added a .slugignore file, so that Heroku ignores everything in the documentation folder, as this is for the README.md & TESTING.md and not required to run the app. |
| 11 | Incorrect category select dropdown | Whilst testing the site on mobile devices, when trying to select different options for the categories the user is unable to select the category they want. | I found a solution on Stack Overflow which stops propagation on a touchend event. The user can now select the correct category dropdown |
| 12 | Admin editing of user's recipes | When admin edits another user's recipes, it changes the created_by field to the admin user as the edit recipe function has been copied from the add recipe function. | By setting the created_by field to who originally created the recipe in the edit recipe function, it no longer overwrites the created_by field with the Admin username. |
